...
function setupEmojiPickers() {
    ...
    emojiPicker.addEventListener('change', () => {
        if (emojiPicker.value) {
            ...
            emojiPicker.value = ''; // Reset selection
            quickEntryEditor.focus();
        }
    });
    
    const templateEmojiPicker = document.getElementById('template-emoji-picker');
    const templateEditor = document.getElementById('template-content');
    
    templateEmojiPicker.addEventListener('change', () => {
        if (templateEmojiPicker.value) {
            // Insert the emoji at cursor position
            document.execCommand('insertText', false, templateEmojiPicker.value);
            templateEmojiPicker.value = ''; // Reset selection
...
// Add this to scripts/app.js after loading settings
